...
//!
//! IOC feeds come in two shapes, and exfil handles both through the ordinary
//! rule/dataset/catalog pipeline — no bespoke storage:
//!
//! - **Content indicators** (domains, IPs, URLs, filenames, strings) are just
//!   regex [`Rule`](exfil_core::Rule)s whose pattern matches the indicator, so
...
//!
//! Because both are `Rule`s, an IOC feed is just a dataset: `exfil pull` /

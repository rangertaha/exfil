//! The exfil graph store: an embedded, pure-Rust SurrealDB (SurrealKV engine).
//!
...
mod tests {
    ...
    async fn opens_and_roundtrips_a_record() {
        let dir = std::env::temp_dir().join(format!("exfil-store-test-{}", std::process::id()));
    ...
    async fn findings_scan_search_and_get() {
        let dir =
            std::env::temp_dir().join(format!("exfil-store-test-full-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    async fn catalog_dataset_crud() {
        let dir = std::env::temp_dir().join(format!("exfil-catalog-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    async fn node_and_edge_crud_roundtrip() {
        let dir = std::env::temp_dir().join(format!("exfil-crud-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let store = Store::open(&dir, DB_FINDINGS).await.unwrap();

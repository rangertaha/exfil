...
use exfill_core::Dataset;

/// A place datasets are fetched from.
#[async_trait]
pub trait Source: Send + Sync {
    /// Stable identifier, shown by `exfill sources`.
    fn name(&self) -> &str;
...
impl Source for BuiltinSource {
    ...
    async fn fetch(&self, reference: &str) -> Result<Dataset> {
        ...
        match name {
            "security" => Ok(Dataset {
                name: "security".into(),
                rules: exfill_scan::builtin_rules(),
            }),
...
mod tests {
    ...
    async fn file_source_reads_native_json() {
        let dir = std::env::temp_dir().join(format!("exfill-src-{}", std::process::id()));

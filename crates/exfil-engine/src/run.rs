...
//!
//! Where a [`Pipeline`](exfil_task::Pipeline) wires per-file plugins by their
...
mod tests {
    ...
    async fn fetch_scan_report_runs_in_order_through_the_graph() {
        let base = std::env::temp_dir().join(format!("exfil-run-test-{}", std::process::id()));
        ...
        let stages: Vec<Box<dyn RunStage>> = vec![
            ...
            Box::new(ScanStage {
                root: tree.clone(),
                pipeline: exfil_scan::default_pipeline().unwrap(),
    ...
    async fn fetch_stage_and_analyze_convenience() {
        let base = std::env::temp_dir().join(format!("exfil-run-conv-{}", std::process::id()));
        let tree = base.join("tree");
    ...
    async fn exfil_engine_scan(tree: &Path, store: &Store, store_dir: &Path) {
    ...
    async fn report_stage_rejects_unknown_format() {
        let base = std::env::temp_dir().join(format!("exfil-run-badfmt-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);

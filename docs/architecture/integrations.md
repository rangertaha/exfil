...
The final layer is how exfill talks to the outside world: an **MCP server** for AI
...
## 1. MCP server — findings for AI agents (`exfill-mcp`)

[MCP](https://modelcontextprotocol.io/) (Model Context Protocol) is the standard by
which AI agents call tools. `exfill mcp` runs a JSON-RPC 2.0 server over stdio that
exposes the findings graph, so an agent (like Claude) can query your scan results
directly.

Source: [`crates/exfill-mcp/src/lib.rs`](../../crates/exfill-mcp/src/lib.rs).

```mermaid
sequenceDiagram
    participant A as AI agent
    participant S as exfill mcp (stdio)
...
- `handle(store, req)` ([`mcp/lib.rs:62`](../../crates/exfill-mcp/src/lib.rs#L62))
...
- `serve(store)` ([`mcp/lib.rs:162`](../../crates/exfill-mcp/src/lib.rs#L162)) is
  the thin stdio loop: read a line, parse JSON, call `handle`, write the response.

**The five tools** ([`mcp/lib.rs:28`](../../crates/exfill-mcp/src/lib.rs#L28)) — all
...
([`mcp/lib.rs:115`](../../crates/exfill-mcp/src/lib.rs#L115)) — visible to the agent
...
`RuleBasedEnricher` ([`llm/lib.rs:31`](../../crates/exfill-llm/src/lib.rs#L31))
...
`run(store, enricher)` ([`llm/lib.rs:73`](../../crates/exfill-llm/src/lib.rs#L73))
...
## 3. Rhai scripting — user triage rules (`exfill-script`)

For custom triage logic you don't want to recompile, `ScriptEnricher`
([`script/lib.rs:24`](../../crates/exfill-script/src/lib.rs#L24)) implements the
...
- **Sandboxed** — Rhai scripts have no filesystem or network access, so running an
  untrusted rule set is safe ([`script/lib.rs:15`](../../crates/exfill-script/src/lib.rs#L15)).
- **Bounded** — `set_max_operations(500_000)` and `set_max_expr_depths(64, 64)`
  ([`script/lib.rs:36`](../../crates/exfill-script/src/lib.rs#L36)) cap a runaway or
  malicious script.
- **Fail-soft at runtime** — a script error (or a non-string result) yields *no
  note* rather than aborting the whole enrichment pass
  ([`script/lib.rs:89`](../../crates/exfill-script/src/lib.rs#L89)); syntax errors,
  by contrast, fail early at compile time.

---

## 4. Remote scanning over SSH (`exfill-remote`) {#remote}

`exfill scan-remote user@host:/path` scans another machine. `exfill-remote`
...
- `RemoteTarget::parse` ([`remote/lib.rs:38`](../../crates/exfill-remote/src/lib.rs#L38))
  handles SCP-style `[user@]host[:/path]`, defaulting the user to `$USER` and the
  path to `/`.
- Auth is key (`-k`, with optional passphrase) or password
  (`$EXFILL_SSH_PASSWORD`) ([`remote/lib.rs:65`](../../crates/exfill-remote/src/lib.rs#L65)).
- The directory walk is **stack-based, not recursive**
  ([`remote/lib.rs:148`](../../crates/exfill-remote/src/lib.rs#L148)) — a `while let
...
## 5. Reporting (`exfill-report`) {#reporting}

Reporters turn an `Analysis` (findings + store counts) into output. Each format is
a `Reporter` ([`report/lib.rs:72`](../../crates/exfill-report/src/lib.rs#L72));
`reporter_for(name)` ([`report/lib.rs:82`](../../crates/exfill-report/src/lib.rs#L82))
picks one.
...
The **JUnit** reporter ([`report/lib.rs`](../../crates/exfill-report/src/lib.rs),
`JunitReporter`) is built for CI: runners like Jenkins, GitLab CI, and GitHub
Actions ingest JUnit XML natively, so `exfill analyze -f junit > results.xml` lets a
...
```mermaid
flowchart LR
    SCAN["exfill scan"] --> AN["exfill analyze -f junit"]
    AN --> XML["results.xml"]
    XML --> CI["CI runner"]
...
([`report/lib.rs:8-11`](../../crates/exfill-report/src/lib.rs#L8)).
...
seam the rest of exfill already defined:
